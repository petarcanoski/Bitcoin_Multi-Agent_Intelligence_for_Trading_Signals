...
def build_prompt_for_agent(agent_name: str, data: Dict[str, Any]) -> str:
    """Build a prompt that asks the LLM for overall score, reasoning, and concrete fixes.

    The prompt requests JSON only with keys: overall (int), reasoning (str), recommendations (list[str]).
    Recommendations should be actionable and prioritized (3 items).
    """
    summary = json.dumps(data, ensure_ascii=False, indent=2)
    prompt = (
        f"You are an impartial technical judge and practical engineering advisor.\n"
        f"Given the automatic evaluation summary for the '{agent_name}' agent, provide THREE outputs:\n"
        "1) overall: a single integer 0-100 (higher is better).\n"
        "2) reasoning: a concise justification (1 sentence).\n"
        "3) recommendations: a prioritized list of 3 concrete, actionable fixes that would most likely improve this agent's score (be specific, e.g. 'increase labeled examples for the high-risk class by N', 'adjust decision threshold to 0.45', 'add ATR(14) volatility feature and retrain').\n\n"
        "Return VALID JSON ONLY with keys: overall (int), reasoning (string), recommendations (array of strings).\n"
        "Do not include any text outside the JSON. Be concise. Try to give as short an answer as possible.\n\n"
        f"Evaluation summary:\n{summary}\n\n"
        "If some metric is missing, mention that in the recommendations."
    )
    return prompt


def call_gemini_v2(prompt: str, model: str = "gemini-2.5-flash", temperature: float = 0.0, max_tokens: int = 4096) -> str:
    try:
...
def evaluate_with_gemini(data: Dict[str, Any], dry_run: bool = True, model: str = "gemini-2.5-flash") -> Dict[str, Any]:
    results = {"meta": {"mode": "dry-run" if dry_run else model}, "judgements": {}}
...
def main(argv: list) -> int:
    ...
    p.add_argument("--out", dest="out_path", default="llm_judgement_gemini_v3.json")
    p.add_argument("--dry-run", dest="dry_run", action="store_true")
    p.add_argument("--model", dest="model", default="gemini-2.5-flash")
